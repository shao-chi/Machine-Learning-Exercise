...
Original file is located at
    https://colab.research.google.com/drive/1FssszhLcZLBPESxHDwMJl96wpHDD-Tn2
"""
...
parser.add_argument("--batch_size", default=64, type=int) # 16 32 64 128
parser.add_argument("--lr", default=0.0003, type=float) # ~0.0001 ~0.001
parser.add_argument("--gamma", default=0.99, type=float) # 0.8 0.9 0.99 0.999
parser.add_argument("--epsilon_start", default=1.0, type=float)
parser.add_argument("--epsilon_final", default=0.01, type=float) # Try 0.05?
parser.add_argument("--epsilon_decay", default=4000000, type=float) # 500000 1000000 4000000 
parser.add_argument("--target_step", default=8000, type=int) # ?
parser.add_argument("--eval_per_ep", default=10, type=int)
parser.add_argument("--save_per_ep", default=50, type=int)
parser.add_argument("--save_dir", default="./drive/My Drive/Colab Notebooks/DL_HW3/model")
parser.add_argument("--log_file", default="./drive/My Drive/Colab Notebooks/DL_HW3/log.txt") # you can plot the figure accroding to the file
...
class DQN(object):
    ...
    def update(self):
        ...
        # expected_state_action_values = state_action_values + self.interaction_steps * (reward_batch + (next_state_values * self.GAMMA) - state_action_values)
        ...
        # next_q_state_values = self.target_net(non_final_next_states).detach()

        # q_value = q_values.gather(1, action_batch.unsqueeze(1)).squeeze(1)
        # next_q_value = next_q_state_values.gather(1, next_q_values.max(1)[1].unsqueeze(1)).squeeze(1)
        # expected_q_value = reward_batch + self.GAMMA * next_q_value * (1 - final_mask)
